...
- [x] **R4-T10** — `adversarial-review/SKILL.md:59` — `$target` is read seventeen times and
...
- [x] **R4-T11** — `validation-rules.md:155` — the `depends_on` check requires an array; both
...
- [x] **R4-T12** — `validation-rules.md:205` — narrowing `taskLines` made it a subset of `ids`,
      ...
      **Acceptance (shape 1)**: evaluate both regexes against a plan with an ID-less task.
      (~9 calls)
- [x] **R4-T13** — `test-guards:158` — a mutation counts as caught whenever the score drops, so
...
- [x] **R4-T14** — `test-count:64` / `test-quiet` — seven of eight planted regressions survived,
...
- [x] **R4-T15** — `reply-to-claude/SKILL.md:100` — the confirm-before-publishing instruction
...
- [x] **R4-T16** — `adversarial-loop/SKILL.md:41` — the flow diagram still states the CONFIRMED
      ...
      **Acceptance (shape 3)**: the old wording is absent AND the new wording is present.
      (~7 calls)
- [x] **R4-T17** — `daily-digest/SKILL.md:103` — the orchestrator is told to point collectors at
...
- [x] **R4-T18** — `CHANGELOG.md:147` — the 2.2.0 entry claims the plugin names no employer and
...
- [x] `./plugin/scripts/test-guards` — corpus, integrity and mutations all green, with the
      corpus grown by the cases above
- [x] Every task's criterion was observed to **fail before** its fix (recorded per task)
- [x] `./plugin/scripts/check` passes
- [x] No finding fixed without a criterion that can fail

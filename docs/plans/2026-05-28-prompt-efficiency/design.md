...
| "Opus 4.8 low effort ≈ 4.7 high" and "cleaner tokens" hold as claimed | **Unverified** — check the official Opus 4.8 model card / published benchmarks before relying on it | Do not bake effort assumptions into commands; treat effort as an operational knob only |
| Claude Code command frontmatter supports an effort hint | Unverified | Keep effort selection out-of-band (user/session setting), not in command frontmatter |

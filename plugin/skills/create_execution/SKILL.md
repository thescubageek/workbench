...
description: Deprecated alias of create_tasks — use /wb:create_tasks (removed at 4.0.0)
...
This command was renamed to `/wb:create_tasks`, matching the convention that a `create_*` stage is named for the artifact it writes — and this one writes `tasks.md`. The alias remains through 3.x and is removed at 4.0.0.
